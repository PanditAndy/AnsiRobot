...
from AnsiRobot import DRAGONS
from AnsiRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Ansi_is_admin(chat_id: int):
    status = False
    Ansi = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Ansi.id == user.id:
